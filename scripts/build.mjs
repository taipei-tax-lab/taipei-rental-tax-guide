...
const tax = (item, type = 'detail') => {
  const noteText = type === 'card' ? (item.summary || item.note) : item.note;
  return `<div class="v2-tax"><dt>${esc(item.label)}</dt><dd><strong>${esc(item.value)}</strong>${noteText ? `<p>${esc(noteText).replace(/\n/g, '<br>')}</p>` : ''}</dd></div>`;
};
...
const otherTaxes = plan => {
  const landTax = plan.taxes[1];
  const incomeTax = plan.taxes[2];
  return `<span>${esc(landTax.label)}：${esc(landTax.cardSummary || landTax.value)}</span><span>${esc(incomeTax.label)}：${esc(incomeTax.cardSummary || incomeTax.value)}</span>`;
};
...
const cards = plans.map((plan, i) => `<a class="v2-plan-card v2-${esc(plan.accent)}" href="#plan-${plan.id}" id="card-${plan.id}">
  ...
  <h3>${esc(plan.situation)}</h3><p class="v2-plan-name">${esc(plan.cardEyebrow || plan.eyebrow)}</p>
  <p class="v2-card-description">${esc(plan.cardDescription || plan.suitable)}</p>
...
const planDetails = plans.map(plan => {
  const sourceList = plan.sourceLinks || [
    { title: plan.source.title, url: plan.source.url },
    ...plan.links.slice(1).map(l => ({ title: l.label, url: l.href }))
  ];
  return `<section data-page="plan-${plan.id}" id="plan-${plan.id}" class="v2-container v2-inner-page v2-${esc(plan.accent)}" aria-labelledby="title-${plan.id}">
  ...
  <section class="v2-source"><h2>官方資訊與申辦入口</h2><div class="v2-source-links">${sourceList.map(l => link({label: l.title || l.label, href: l.url || l.href})).join('')}</div></section>
...
const faqs = [
  [
    '房客申請租金補貼，房東可以有哪些租稅優惠？',
    '可先了解公益出租人。也可查看四方案完整比較，了解社會住宅包租代管。',
    [
      {href:'#plan-public', label:'查看公益出租人的優惠與認定方式'},
      {href:'#comparison', label:'查看四方案完整比較'}
    ]
  ],
  [
    '「包租」與「代管」有什麼不同？',
  ...
  [
    '享受租稅優惠要自己申請，還是機關會主動辦理？',
    '公益出租人與社會住宅包租代管由稅捐處依臺北市政府都市發展局造冊資料主動辦理；出租達租金標準及個人租賃住宅包租代管則需由屋主主動提出申請（房屋稅 3/22 前、地價稅 9/22 前，逾期申請自次年(期)起適用）。另提醒您，綜合所得稅請依相關規定向國稅局辦理申報。',
    []
  ]
];
const faqHtml = faqs.map(([q, a, actions = []]) => {
  const actionsHtml = actions.map(action => action.href.startsWith('#')
    ? `<a class="v2-text-link" href="${esc(action.href)}">${esc(action.label)} →</a>`
    : link(action, 'v2-text-link')).join('');
  return `<details><summary><span>${esc(q)}</span><span class="v2-expand" aria-hidden="true">＋</span></summary><div class="v2-details-body"><p>${esc(a)}</p>${actionsHtml}</div></details>`;
}).join('\n');
